...
class ShapeAnalysis:
	...
	def analysis(self, frame):
		...
		cv.imwrite("D:/test-result"+str(i)+".png", self.draw_text_info(result))
		return self.shapes
	...
	def draw_text_info(self, image):
		...
		cv.putText(image, "square: " + str(c5), (10, 100), cv.FONT_HERSHEY_PLAIN, 1.2, (255, 0, 0), 1)
		return image
...
if __name__ == "__main__":
		...
		while True:
			...
			if cv.waitKey(1)&0xFF==ord('s'):
				cv.imwrite(r"D:/"+str(i)+".png",frame)
				src=cv.imread(r"D:/"+str(i)+".png")    
				i += 1
				ld = ShapeAnalysis()
				ld.analysis(src)
